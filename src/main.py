...
from src.ssz.merkle import get_fixed_capacity_proof, merkle_list_tree
...
def generate_validator_proof(
    ...
    electra = False
    if state.fork.current_version == bytes.fromhex("05000000"):
        electra = True
    ...
    if not electra:
        ...
        if validator_index >= len(state.validators):
            raise ValueError(
                f"Validator index {validator_index} out of range (max: {len(state.validators)-1})"
            )
    ...
    val_proof = get_fixed_capacity_proof(
        validator_elements, validator_index, VALIDATOR_REGISTRY_LIMIT
    )
    ...
    validators_root = compute_root_from_proof(leaf, validator_index, val_proof)

    # Generate state proof for validators field (field index 9)
    state_proof = _generate_state_proof(
        state,
        field_index=9,
        prev_state_root=prev_state_root_bytes,
        prev_block_root=prev_block_root_bytes,
    )
    ...
    validator_balance = ValidatorBalance(
        validator=validator, balance=state.balances[validator_index]
    )
...
def generate_balance_proof(
    ...
    if validator_index >= len(state.balances):
        raise ValueError(
            f"Validator index {validator_index} out of range (max: {len(state.balances)-1})"
        )
    ...
    balance_proof = get_fixed_capacity_proof(balance_chunks, chunk_index, limit)
    ...
    balances_root = compute_root_from_proof(leaf, chunk_index, balance_proof)
    ...
    # Create ValidatorBalance container
    validator_balance = ValidatorBalance(validator=validator, balance=balance)
...
def _generate_state_proof(
    state: BeaconState,
    field_index: int,
    prev_state_root: Optional[bytes] = None,
    prev_block_root: Optional[bytes] = None,
) -> List[bytes]:
    """
    ...
    if state.fork.current_version == bytes.fromhex("05000000"):
        electra = True
    state_fields = state.serialize(prev_block_root, prev_state_root, is_electra=electra)
...
def _compute_state_root(
    state: BeaconState, validators_root: Optional[bytes] = None
) -> bytes:
...
def generate_merkle_witness(
    ...
    if not electra:
        ...
        if prev_state_root_bytes is None or prev_block_root_bytes is None:
            try:
                ...
                if prev_state_root_bytes is None:
                    prev_state_root_hex = state_8_data["state_roots"][2][
                        2:
                    ]  # Remove 0x prefix
                    prev_state_root_bytes = bytes.fromhex(prev_state_root_hex)
                if prev_block_root_bytes is None:
                    prev_block_root_hex = state_8_data["block_roots"][2][
                        2:
                    ]  # Remove 0x prefix
                ...
                if prev_state_root_bytes is None:
                    prev_state_root_bytes = bytes.fromhex(
                        "01ef6767e8908883d1e84e91095bbb3f7d98e33773d13b6cc949355909365ff8"
                    )
                if prev_block_root_bytes is None:
                    prev_block_root_bytes = bytes.fromhex(
    ...
    validator_proof = get_fixed_capacity_proof(
        validator_tree, current_index, VALIDATOR_REGISTRY_LIMIT
    )
