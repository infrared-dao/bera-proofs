...
class BeaconAPIClient:
    ...
    def get_beacon_state(self, slot: str = "head") -> Dict[str, Any]:
        ...
        try:
            ...
            raise BeaconAPIError(
                f"Request failed to beacon API at {self.base_url}. " f"Error: {e}"
            )
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                ...
                if isinstance(value, dict):
                    ...
                    sanitized[snake_key] = [
                        self.sanitize_beacon_data(item)
    ...
    def get_historical_roots(
        self, current_slot: int, slots_back: int = 8
    ) -> tuple[str, str]:
        """
        ...
        try:
            ...
            logger.info(
                f"Retrieved historical data for slot {historical_slot} (current-{slots_back})"
            )
            return state_root, parent_root

        except Exception as e:
            logger.warning(
                f"Could not fetch historical data for slot {historical_slot}: {e}"
            )
    ...
    def get_state_with_historical_data(
        self, slot: str = "head", include_historical: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if include_historical:
            try:
                ...
                current_slot = int(
                    state_data.get("slot", "0"),
                    16
                    if isinstance(state_data.get("slot"), str)
                    and state_data.get("slot").startswith("0x")
                    else 10,
                )

                # Get historical roots
                prev_state_root, prev_block_root = self.get_historical_roots(
                    current_slot
                )
    ...
    def create_test_data_with_historical(
        self, base_state_file: str, historical_slot_offset: int = 8
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                prev_state_root, prev_block_root = self.get_historical_roots(
                    current_slot, historical_slot_offset
                )
            except:
                # Use test defaults
                prev_state_root = (
    ...
    def extract_historical_roots_from_state(
        ...
        if "slot" in state_data:
            try:
                current_slot = int(
                    state_data["slot"],
                    16 if state_data["slot"].startswith("0x") else 10,
                )
