...
async def get_validator_proof(
    val_index: int,
    prev_state_root: Optional[str] = Query(
        None,
        description="Previous state root from 8 slots ago (hex string, auto-fetched if not provided)",
    ),
    prev_block_root: Optional[str] = Query(
        None,
        description="Previous block root from 8 slots ago (hex string, auto-fetched if not provided)",
    ),
    json_file: Optional[str] = Query(None, description="Path to JSON state file"),
    slot: str = Query("head", description="Slot number"),
    auto_fetch_historical: bool = Query(
        True, description="Automatically fetch historical data if not provided"
    ),
):
