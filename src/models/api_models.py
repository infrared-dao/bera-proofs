...
class ProofRequest(BaseModel):
    ...
    prev_state_root: Optional[str] = Field(
        default=None, description="Previous state root from 8 slots ago (hex string)"
    )
    prev_block_root: Optional[str] = Field(
        default=None, description="Previous block root from 8 slots ago (hex string)"
    )
    ...
    def validate_prev_state_root(cls, v):
        ...
        if v is not None and (not v.startswith("0x") or len(v) != 66):
            raise ValueError(
                "prev_state_root must be a 32-byte hex string starting with '0x'"
            )
        return v
    ...
    def validate_prev_block_root(cls, v):
        ...
        if v is not None and (not v.startswith("0x") or len(v) != 66):
            raise ValueError(
                "prev_block_root must be a 32-byte hex string starting with '0x'"
            )
        return v
...
class ProofResponse(BaseModel):
    ...
    proof_type: str = Field(
        ..., description="Type of proof (validator/balance/proposer)"
    )
    metadata: dict = Field(
        default_factory=dict, description="Additional proof metadata"
    )
    ...
    def validate_proof_format(cls, v):
        ...
        for step in v:
            if not isinstance(step, str) or not step.startswith("0x"):
                raise ValueError(
                    "All proof steps must be hex strings starting with '0x'"
                )
        return v
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "metadata": {"proof_length": 45, "validator_count": 100},
            }
        }
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "slot": "head",
                "proof_type": "balance",
                "metadata": {"proof_length": 45, "balance": "32000000000"},
            }
        }
